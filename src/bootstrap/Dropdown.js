...
define([ 
	...
	var Dropdown = declare([],{
		...
		constructor: function(element, options){
			...
			if(!el[0]){ el = query(element); }
			if(el){ domAttr.set(el[0], "data-toggle", "dropdown"); }
		}
	});
	function toggle(e){
		var _this = this;
		if(domClass.contains(_this, "disabled") || domClass.contains(_this, ":disabled")){ return false; }
	...
	lang.extend(query.NodeList, {
		dropdown: function(option){
			...
			return this.forEach(function(node){
				var data = nodeData.get(node, 'dropdown');
				if(!data){ nodeData.set(node, 'dropdown', (data = new Dropdown(node, options))) }
