...
define([ 
	...
	'dojo/domReady!' 
], function (declare, query, lang, win, on, domClass, domConstruct, domAttr, trans, nodeData, domGeom, domStyle){
	...
	var Scrollspy = declare([],{
		...
		refresh: function(){
	        ...
	        query(this.selector, win.body()).map(function(node){
				nodeData.load(node);
				var href = nodeData.get(node, 'target') || domAttr.get(node, 'href');
				var target = /^#\w/.test(href) && query(href)[0];
				var pos;
				if(target){ pos = domGeom.position(target, false); }
		...
		process: function(){
	        ...
	        var scrollTop = this.element.scrollTop + parseInt(this.options.offset);
...
//console.log(offsets);
	...
	lang.extend(query.NodeList, {
		scrollspy: function(option){
			...
			return this.forEach(function(node){
				var data = nodeData.get(node, 'scrollspy');
				if(!data){ nodeData.set(node, 'scrollspy', (data = new Scrollspy(node, options))) }
	...
	query(spySelector).forEach(function(node){
		nodeData.load(node);
		query(node).scrollspy(nodeData.get(node));
	});
